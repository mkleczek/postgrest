...
-- | Run an application under the process handover framework and install a
-- SIGHUP handler once the application reports readiness.
runRestartableWithSIGHUP ::
  ReplacementConfig ->
  (HandoverMode -> (forall b. IO b -> IO ()) -> IO a) ->
  IO a
runRestartableWithSIGHUP replacementCfg runApp =
  runRestartable replacementCfg $ \mode ready' ->
    runApp mode $ \stopAction ->
      void $ ready' stopAction installSIGHUPHandler
...
installSIGHUPHandler :: IO a -> IO ()
installSIGHUPHandler requestRestart =
  void $ installHandler sigHUP (Catch $ void requestRestart) Nothing
